...
import io.smallrye.jws.JwsVerifier;
                ...
                if (requestData.containsKey(OidcUtils.DPOP_PROOF_JWT_HEADERS)) {
                    result = result.onItem().transform(new Function<TokenVerificationResult, TokenVerificationResult>() {
                        ...
                        public TokenVerificationResult apply(TokenVerificationResult t) {
                            ...
                            JsonWebKey jwk;
                            try {
                                jwk = JsonWebKey.jwk(jwkProof.getMap());
                            ...
                            try {
                                jwkProofThumbprint = jwk.computeThumbprint();
                            } catch (JsonWebKeyException ex) {
                            ...
                            try {
                                JwsVerifier.builder(dpopVerifyKey)
                                        .algorithms(OidcProvider.ASYMMETRIC_ALLOWED_ALGORITHMS)
                                        .build()
                                        .verify((String) requestData.get(OidcUtils.DPOP_PROOF));
