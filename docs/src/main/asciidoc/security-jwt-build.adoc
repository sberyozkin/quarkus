...
The SmallRye JWT Build API simplifies securing JWT claims by supporting all these options. It uses the link:https://connect2id.com/products/nimbus-jose-jwt[Nimbus JOSE + JWT] library internally to provide this functionality.
...
TIP: The Quarkus CLI can generate PEM-encoded RSA or EC key pairs and configure `application.properties` for you: `quarkus jwt generate-key`. See xref:security-jwt.adoc#generating-keys-with-the-quarkus-cli[Generating keys with the Quarkus CLI] for details.
...
* link:https://connect2id.com/products/nimbus-jose-jwt[Nimbus JOSE + JWT]
