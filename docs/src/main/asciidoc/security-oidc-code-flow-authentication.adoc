...
TIP: For development and testing with Keycloak, see xref:security-spiffe-client.adoc#keycloak-dev-svc-integration[SPIFFE integration with Dev Services for Keycloak].
...
[[claims-validator-code]]
==== Claims validator

You can register a custom claims validator to customize the JWT claim verification process. See the xref:security-oidc-bearer-token-authentication.adoc#claims-validator-bearer[Claims validator] section for more information.
...
* xref:security-oidc-client-id-metadata-document.adoc[OAuth Client ID Metadata Document]
