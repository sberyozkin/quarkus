...
[[claims-validator-bearer]]
=== Claims validator

You can register a custom `io.smallrye.jwt.auth.ClaimsValidator` to customize the JWT claim verification process, before `org.eclipse.microprofile.jwt.JsonWebToken` is initialized.
For example:
...
----
<1> Register the claims validator to verify JWT tokens for all OIDC tenants.
...
`VerificationContext` is a record declared by `ClaimsValidator`, and its `claims()` accessor returns an `io.smallrye.jwt.common.JwtClaims` instance, which offers both the generic `get(String)` and `containsKey(String)` methods and the standard claim accessors such as `getIssuer()` and `getAudience()`.

TIP: Use a `@quarkus.oidc.TenantFeature` annotation to bind a custom claims validator to a specific OIDC tenant only.
...
Use the `quarkus.oidc.dpop.proof-age` property to reject DPoP proofs older than the configured duration (default is 2 minutes), calculated from the proof's `iat` (issued at) claim.
This ensures proofs with timestamps in the past or future are properly rejected. The `quarkus.oidc.token.lifespan-grace` property can optionally add a grace period to tolerate slight clock skews.
...
Or, if you need more flexibility, write a <<claims-validator-bearer>>:
...
public class AcrValueValidator implements ClaimsValidator {
    ...
    @Override
    public String validate(VerificationContext context) {
        JwtClaims jwtClaims = context.claims();
        String acrClaimName = Claims.acr.name();

        if (jwtClaims.containsKey(acrClaimName)) {
            // The claim 'acr' could be a String or a list
            Object acrClaimValue = jwtClaims.get(acrClaimName);
            List<String> acrClaimValues;
            if (acrClaimValue instanceof List) {
               acrClaimValues = (List<String>) acrClaimValue;
            } else if (acrClaimValue instanceof String) {
               acrClaimValues = List.of((String) acrClaimValue);
            } else {
                return "Claim '" + acrClaimName + "' is not a String or List of Strings.";
            }
