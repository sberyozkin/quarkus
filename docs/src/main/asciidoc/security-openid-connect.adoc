...
Note that if the OpenId Connect Provider token endpoint URL is not based on the configured `quarkus.oidc.auth-server-url` value used by the Quarkus endpoint then an issuer verification failure may be reported by the endpoint. In such cases, if you work with Keycloak, please start it with a `KEYCLOAK_FRONTEND_URL` system property set to the base URL used by SPA.
