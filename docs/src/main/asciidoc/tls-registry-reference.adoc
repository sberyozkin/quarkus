...
Your Quarkus HTTPS application must use a _build-time_ property to enable a Let's Encrypt ACME challenge management route:
...
TLS registry can serve ACME challenge requests from either the main or management router.
Using a management router is **strongly** recommended to let Quarkus deal with ACME challenge requests separately to the main application's deployment and security requirements:
...
Finally, configure TLS registry to use a certificate and private key from a `.letsencrypt` directory relative to your application's root directory:

[source, properties]
----
quarkus.tls.key-store.pem.acme.cert=${your-application-root-directory}/.letsencrypt/lets-encrypt.crt
quarkus.tls.key-store.pem.acme.key=${your-application-root-directory}/.letsencrypt/lets-encrypt.key
quarkus.tls.lets-encrypt.enabled=true
quarkus.management.enabled=true
----

Do not start your application yet. Use CLI in the next step to create and prepare a `.letsencrypt` directory for you first.

=== Use CLI to prepare your application
...
[source, shell]
----
quarkus tls-registry lets-encrypt prepare --domain=<domain-dns-name>
----
...
You can start your aplication in devmode:

include::{includes}/devtools/dev.adoc[]

Access your application endpoint using `https://localhost:8443/`, for example, `https://localhost:8443/hello`, accept a self-signed domain certificate in the browser.

Next, request your first Let's Encrypt certificate.

[[lets-encrypt-issue-certificate]]
=== Issue certificate

Run the `issue-certificate` command to acquire your first Let's Encrypt certificate:

[source, shell]
----
quarkus tls-registry lets-encrypt issue-certificate
...
During this command, TLS registry CLI checks a Quarkus management router's readiness, creates and records a Let's Encrypt account information, issues a Let's Encrypt certificate request, and communicates with a Quarkus management router to have ACME challenges resolved.

Once the Let's Encrypt certificate chain and private key have been successfully acquired, they are converted to PEM format and copied to your application's `.letsencrypt` folder. TLS registry is informed that a new certificate and private key are ready and it reloads them automatically.

Now, access your application's endpoint using `https://localhost:8443/` again. Confirm in the browser that your domain certificate is now signed by the Let's Encrypt certificate authority.
...
[source, shell]
----
quarkus tls-registry lets-encrypt renew-certificate
--domain=<domain-dns-name> <1>
----
<1> Set your domain name.

During this command, TLS registry CLI reads a Let's Encrypt account information recorded during the <<lets-encrypt-issue-certificate>> step, issues a Let's Encrypt certificate request, and communicates with a Quarkus management router to have ACME challenges resolved.
...
Using Ngrok provides an eaisest option to get started with the Quarkus Let's Encrypt ACME feature.
...
`YOUR-RESERVED-DOMAIN-ID` is your reserved domain's id which starts from `rd_`, you can find in the https://dashboard.ngrok.com/cloud-edge/domains[NGrok dashboard domains section].
...
You can now test the the Quarkus Let's Encrypt ACME feature on your localhost.
