...
|quarkus.oidc.credentials.jwt.public-key ||The inlined public key (PEM or certificate)
|quarkus.oidc.credentials.jwt.public-key-file ||The public key file (PEM or certificate)
|====
...
`quarkus.oidc.credentials.jwt.public-key` and `quarkus.oidc.credentials.jwt.public-key-file` are only meaningful in the context of <<client-id-metadata-document,Client ID Metadata Document>> registration.
When a private key is configured and the client-id is a <<client-id-metadata-document,CIMD>> HTTPS URL, the corresponding public key is published in the metadata document's `jwks` property, and `token_endpoint_auth_method` is automatically set to `private_key_jwt`, enabling asymmetric client authentication without pre-registration.
...
See also the <<custom-claims-validator>> section for details about customizing the JWT token verification.
...
See the xref:security-oidc-bearer-token-authentication.adoc#demonstrating-proof-of-possession-dpop[Demonstrating Proof of Possession for access tokens] section of the xref:security-oidc-bearer-token-authentication.adoc[OpenID Connect Bearer token authentication] guide for more information.
...
You can register a custom xref:security-oidc-bearer-token-authentication.adoc#claims-validator-bearer[claims validator] to verify the JWT token content, in addition to the checks described in the <<token-verification>> section above.
...
The link:https://datatracker.ietf.org/doc/html/draft-ietf-oauth-client-id-metadata-document[OAuth Client ID Metadata Document (CIMD)] specification enables OAuth clients to use an HTTPS URL as their `client_id`. That URL points to a JSON document containing the client's metadata, which the authorization server fetches on demand instead of requiring the client to be pre-registered.

CIMD is enabled automatically for Quarkus OIDC web applications whose OIDC `quarkus.oidc.client-id` is a valid HTTPS URL, when other required properties such as `quarkus.oidc.client-name` and `quarkus.oidc.authentication.redirect-path` are also set.

.Client ID Metadata Document properties
[options="header"]
|====
|Property name |Default |Description

|quarkus.oidc.cimd.force-https-scheme |true|Force that a client id set as CIMD URL has an HTTPS scheme
|====

The CIMD URL scheme must be set to `HTTPS` by default. You can enable an `HTTP` URL scheme with `quarkus.oidc.cimd.force-https-scheme=false`, it can be particularly useful in simple demos and tests.

[NOTE]
====
If you know that neither static nor dynamic OIDC tenants will support CIMD, you can disable an HTTP route that supports CIMD requests to avoid unnecessary overhead, see the <<allowed-routes>> section for more information.
====

To learn more about Client ID Metadata Document support, see the xref:security-oidc-client-id-metadata-document.adoc[OAuth Client ID Metadata Document] guide.
...
|quarkus.oidc.allowed-routes |`backchannel-logout,resource-metadata,client-id-metadata`|OIDC routes that tenants can use
|====

To be able to support https://openid.net/specs/openid-connect-backchannel-1_0.html[OIDC back-channel logout], https://datatracker.ietf.org/doc/rfc9728/[OAuth2 Protected Resource Metadata] and https://datatracker.ietf.org/doc/html/draft-ietf-oauth-client-id-metadata-document[OAuth Client ID Metadata Document] requests, `quarkus-oidc` must register routes that require dedicated HTTP handlers for all static and dynamic OIDC tenants that must support all or one of these features.
...
* `client-id-metadata` - enables an HTTP route for OIDC tenants to handle requests for <<client-id-metadata-document,Client ID Metadata Document>>
